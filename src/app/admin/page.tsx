...
export default function AdminPage() {
  ...
  // CRUD de productos (simulado con estado local)
  ...
  const handleToggleHabilitado = async (prod: Producto) => {
    ...
    try {
      ...
      await fetchProductos(); // Recarga la lista para reflejar el cambio
    } catch (e: any) {
  ...
  // Agrupa categorías normalizadas y muestra solo una vez cada una, pero omite 'Helado' (singular) y 'Bebidas / Tragos'
  const categoriasMap = new Map<string, string>();
  productos.forEach(p => {
    const norm = normalizar(p.categoria);
    // Solo agrega si no es 'helado' (singular) ni 'bebidas / tragos' (normalizado)
  ...
  if (!autenticado) {
    return (
      <main className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-stone-900 via-neutral-900 to-zinc-900 px-4 py-16 font-serif animate-fade-in">
        <h2 className="text-4xl font-serif font-extrabold text-stone-200 mb-4 drop-shadow-xl text-center">Iniciar sesión</h2>
        <p className="text-stone-400 text-center mb-6 max-w-md mx-auto">Esta sección es solo para administradores. Si eres cliente, puedes navegar y hacer pedidos sin iniciar sesión.</p>
